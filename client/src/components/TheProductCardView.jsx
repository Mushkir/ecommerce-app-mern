...
const TheProductCardView = ({ loading, data }) => {
  // console.log(data);
  ...
  return (
    <div className="grid grid-cols-3 justify-center md:justify-between gap-5 md:gap-4">
      {loading ? (
        <TheListSkeleton
          listsToRender={10}
          content={
        ...
        data.map((product) => {
          ...
          return (
            <Link
              ...
              key={product?._id}
              className=" bg-white w-full min-w-[19rem] md:min-w-[20rem] h-[28rem] rounded overflow-hidden shadow-xl"
            >
              {/* Img */}
...
TheProductCardView.propTypes = {
  loading: PropTypes.bool,
  data: PropTypes.array,
};

export default TheProductCardView;
